...
t = tqdm(total=len(lng_range) * len(lat_range) * 10**8)
                        ...
                        for lat_degree2 in [degree for degree in range(0,9)]:
                            for lng_degree3 in [degree for degree in range(0,9)]:
                                for lat_degree3 in [degree for degree in range(0,9)]:
                                    for lng_degree4 in [degree for degree in range(0,9)]:
                                        for lat_degree4 in [degree for degree in range(0,9)]:
                                            if lng_degree4 > 0 or lat_degree4 > 0:
                                                # prev precision degrees were already written to data{0,1,2,3}
                                                lng_ = lng + lng_degree1/10**1 + lng_degree2/10**2 + lng_degree3/10**3 + lng_degree4/10**4
                                                lat_ = lat + lat_degree1/10**1 + lat_degree2/10**2 + lat_degree3/10**3 + lat_degree4/10**4
                                                tz = get_tz(lng_, lat_)
                                                if tz:
